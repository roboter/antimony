...
win32 {
    ...
    QMAKE_CXXFLAGS += $$system(pkg-config --cflags python3)
    QMAKE_CFLAGS += $$system(pkg-config --cflags python3)
    LIBS += $$system(pkg-config --libs python3)
    LIBS += -lboost_python314-mt
    LIBS += $$system(pkg-config --libs libpng)
    LIBS += -lopengl32
}
